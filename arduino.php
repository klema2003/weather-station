...
<?php
$con=mysqli_connect("localhost","root"," ","arduino"); //Localhost, Username, Password, Database Name
$result=mysqli_query($con,'SELECT * FROM temp ORDER BY id DESC'); // Run SELECT query
while($row=mysqli_fetch_array($result)) // Fetch the table from the Database
{
echo"<tr>";
echo"<td bgcolor=#0066FF>".$row['id']."</td>";
echo"<td bgcolor=#6699FF>".$row['cas']."</td>";
echo"<td bgcolor=#99CCFF>".$row['teplota']."</td>";
echo"<td bgcolor=#99CCFF>".$row['vlhkost']."</td>";
echo"<tr>";
}
